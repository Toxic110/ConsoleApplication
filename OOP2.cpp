...
bool goToScienceLab(Person pers)
{
    if (pers.getAccess() == 0)
    {
        cout << "Access granted for " << pers.getName() << endl;
    }
    else
    {
        cout << "Access denied for " << pers.getName() << endl;
    }
}
...
void buySandwich(Person pers)
{
    ...
    cout << pers.getName() << " buy sandwich for " << round(price*100)/100 << endl;
}
